...
class BaseChunker(ABC):
    ...
    def __init__(self, chunk_size: int = 1000, chunk_overlap: int = 0):
        ...
        logger.info(f"Initialized {self.chunker_name} chunker with size={chunk_size}, overlap={chunk_overlap}")
    ...
    def rank_chunks(self, chunks: List[str], query: str = None) -> List[str]:
        """
    ...
    def validate_parameters(self) -> bool:
        ...
        if self.chunk_overlap >= self.chunk_size:
            logger.error(f"chunk_overlap ({self.chunk_overlap}) must be less than chunk_size ({self.chunk_size})")
            return False
