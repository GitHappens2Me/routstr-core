...
class FixedSizeChunker(BaseChunker):
    ...
    async def chunk_text(self, text: str, **kwargs) -> List[str]:
        ...
        if text_length <= self.chunk_size:
            logger.debug(f"Text length ({text_length}) <= chunk_size ({self.chunk_size}), returning single chunk")
            return [text]
        ...
        while start < text_length:
            ...
            if end > text_length and len(chunks) > 0:
                ...
                if len(remaining_text) < self.chunk_size * 0.3:  # Less than 30% of chunk size
                    chunks[-1] += remaining_text
                    logger.debug(f"Appended remaining {len(remaining_text)} chars to last chunk")
                    break
